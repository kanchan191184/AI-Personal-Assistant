...
# Import the tools
from source.tools.weather_tools import WeatherTools
from source.tools.calendar_tools import CalendarTools
from source.tools.email_tools import GmailTools
from source.tools.search_tools import SearchTools
from source.utils.google_auth import get_gmail_service
# Import prompts
...
from source.agents.agent_factory import create_specialist_agent
...
async def create_weather_agent(model_client: OpenAIChatCompletionClient) -> AssistantAgent:
    ...
    return await create_specialist_agent(
        ...
        tool_factory=WeatherTools().as_function_tools,
    )
...
async def create_calendar_agent(model_client: OpenAIChatCompletionClient) -> AssistantAgent:
    ...
    return await create_specialist_agent(
        ...
        tool_factory=CalendarTools().as_function_tools,
    )
...
async def create_search_agent(model_client: OpenAIChatCompletionClient) -> AssistantAgent:
    ...
    return await create_specialist_agent(
        ...
        tool_factory=SearchTools().as_function_tools,
    )
...
class PersonalAssistantOrchestrator:
    ...
    async def process_request(self, user_input: str) -> str:
        """Process user request with appropriate agent strategy, logging agent identities via .source if available."""
        try:
